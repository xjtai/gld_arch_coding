...
<!-- ──────────────────────────────────────────────────────────────
  prd.md ｜ 广小联企业级高可用智能客服 Agent（培训实战需求）

  为什么是这份文件      这是一次分组实战：学员要把 AI Coding / SDD
        方法论，在一个"看得见、验得了"的题目上跑通一次完整闭环——
        从这份 PRD 出发，spec.md / plan.md / tasks.md 都由 Coding
        Agent 起草，人只做评审（纠错 + 补充），再交给 Coding Agent
        编码、评测、部署，最后把每一轮评审沉淀成 learnings.md。
...
# PRD-2026-GXL-WORKSHOP-001｜广小联企业级高可用智能客服 Agent

> 提出：AI 全流程实战落地工作坊教学组　场景：广小联 AI 全流程实战落地工作坊　用时：约 3 小时分组实战

## 1. 背景

本次分组实战：每组用 AI 编码工具，从一份 PRD 出发，独立完成一个**广小联企业级高可用智能客服 Agent**的需求理解、架构设计与实现，并现场演示。

选客服场景的原因：

- **零门槛**：意图分类一看就懂，不用先花时间铺垫业务，所有人起点一样——注意力能全放在方法上，而不是纠缠"这个业务规则到底该怎么算"；
- 天然适合"多智能体协作"这个训练重点——单一 Agent 硬扛所有意图会又臃肿又难兜底，必须做路由与职责拆分；
- "高可用"这个企业级要求，能强制学员在 3 小时内也要考虑异常兜底、可观测性，而不是只做一条 happy path。

> **练的不是客服机器人。** 这个练习里的每个环节对应你们自己系统里的什么，见 [`README.md` 开头的对照表](./README.md)。
...
**做一个多智能体架构的智能客服 Agent：用户输入一句话，系统能自动判断意图归属的类别，路由给对应的专业智能体处理，并给出正确、可用的答复。**
