...
题目：《广小联企业级高可用智能客服 Agent》。详细需求见 [`prd.md`](./prd.md)。

## 先说清楚：练的不是客服机器人

题目选了客服场景，只因为**它零门槛**——意图分类一看就懂，不用先花半小时铺垫业务，所有人起点一样。**真正要练的是方法**，而方法跟你手上那套系统是通的：

| 这个练习里的 | 对应你们系统里的 |
|---|---|
| 路由智能体 | 任何"把请求分发到不同处理域"的入口层 |
| 多智能体职责划分 | 服务 / 模块 / 领域的边界怎么切 |
| `spec.md` §2 业务语义消歧 | 需求里那些"你以为大家理解一致"的词 |
| `plan.md` §6 不改清单 | 改存量系统时的兼容性硬约束 |
| mock 工具 + 故障注入开关 | 依赖外部系统时的容错与降级设计 |
| `eval.md` 的门禁与判定 | 上线前凭什么说"这次可以发" |
| `learnings.md` 的回流 | 踩过的坑怎么变成团队规范，而不是烂在复盘会里 |

所以做的时候，**别把注意力放在"客服该怎么答"上**——那不是考点。注意力应该在：Agent 起草的东西哪里不对、你凭什么判断它不对、以及这个判断该沉淀到哪儿。
